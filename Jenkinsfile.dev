                    ...
                    sh """
                        ...
                        if ! docker ps --filter "name=nginx_lb" --filter "status=running" | grep -q "nginx_lb"; then
                            ...
                            if [ -f /opt/redis-certs/redis.key ]; then
                                sudo chmod 640 /opt/redis-certs/redis.key
                                # Redis 컨테이너의 그룹 ID(보통 redis=999)가 읽도록 그룹 변경 시도
                                echo "Attempting to set group ownership for redis.key..."
                                sudo chgrp redis /opt/redis-certs/redis.key || sudo chgrp 999 /opt/redis-certs/redis.key || echo "WARN: Group 'redis' or GID '999' not found, relying on 640 permissions."
                            fi
