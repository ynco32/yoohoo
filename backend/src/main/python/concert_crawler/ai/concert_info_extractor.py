...
"""
        ...
        prompt = f"""
다음 텍스트에서 공연 정보를 추출해 설명 없이 정확한 JSON 형식으로만 반환해주세요:

{{
  "concert_name": "공연 이름 (없으면 null)",
  "venue": "공연장 (없으면 null)",
  "artist": "아티스트 이름 (없으면 null)",
  "advance_reservation": "사전 예매 시작일 (형식: 2025-05-01T12:00:00, 없으면 null)",
  "reservation": "일반 예매 시작일 (형식: 2025-05-01T12:00:00, 없으면 null)",
  "start_times": ["공연 시작 시간 (형식: 2025-05-01T18:00:00)", "..."]
}}

중요: 
1. 모든 날짜/시간은 "YYYY-MM-DDThh:mm:ss" ISO 형식으로 작성 (예: "2025-05-01T12:00:00")
2. 'T'로 날짜와 시간을 구분 (공백 아님)
3. 날짜 정보가 없으면 null 사용, 시작 시간이 없으면 빈 배열 []
4. 공연 정보가 명확히 없거나 추론이 불가능하면 해당 필드는 null 또는 빈 배열
5. 반드시 유효한 JSON만 반환 (설명 없이)
