...
const TicketingArea: React.FC = () => {
  ...
  const handleMove = async () => {
    ...
    if (selectedSection) {
      try {
        await router.push(`/ticketing/melon-mode/real/${selectedSection}`);
      } catch (error) {
        console.error('라우팅 에러:', error);
        window.location.href = `/ticketing/melon-mode/real/${selectedSection}`;
      }
    }
    setIsPopupOpen(false);
  };
  ...
  return (
    <div className="max-w-xl w-full p-4">
      <svg viewBox="0 0 205 209" className="w-full">
        {/* Stage */}
        <path
        ...
        {/* A Section Left */}
        <g onClick={() => handleSectionClick('A')} className="cursor-pointer">
          <path
            ...
            fill={selectedSection === 'A' ? '#4F46E5' : '#88DBF1'}
        ...
        {/* B Section Right */}
        <g onClick={() => handleSectionClick('B')} className="cursor-pointer">
          <path
            ...
            fill={selectedSection === 'B' ? '#4F46E5' : '#A08AFF'}
            className="transition-all duration-300 hover:fill-indigo-100"
          />
          <text
            x="148"
        ...
        {/* C Section */}
        <g onClick={() => handleSectionClick('C')} className="cursor-pointer">
          <path
            ...
            fill={selectedSection === 'C' ? '#4F46E5' : '#D1BD9F'}
            className="transition-all duration-300 hover:fill-indigo-100"
          />
          <text
            x="103"
