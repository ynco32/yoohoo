...
2. 원본 내용을 누락하지 말고 모든 정보를 포함해주세요.
3. OCR 오류는 수정하되, 내용을 재구성하거나 요약하지 마세요.
4. 원본 의미는 그대로 유지해주세요.
5. 표 형태 데이터도 검색이 용이하도록 일반 텍스트로 변환하여 포함시키세요:
   * 예시: "본인 확인 가능한 유효 신분증: 주민등록증, 운전면허증, 여권, 청소년증, 외국인등록증 등"
6. 텍스트는 자연스러운 문장 형태로 유지하세요.
7. 각 그룹의 주제나 내용을 대표하는 카테고리를 지정해주세요 (예: 티켓 배송, 현장 수령, 신분증 정보)
